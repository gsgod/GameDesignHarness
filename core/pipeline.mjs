...
import { generationFiles, checkKnownAI } from './generation.mjs';
...
export function plan(root) {
  ...
  for (const a of c.assets) {
    ...
    if (a.provenance.ai) generationFiles(root, a);
  }
  const result = { project: c.project, pack: c.pack, profile: c.profile, engine: c.engine,
    ...
    policy: c.policy,
    capabilities: ['PNG validation', 'contact sheet', 'source and license manifest', 'AI evidence and free-cost gate', 'Godot PNG decode check', 'explicit approval', 'immutable local publish'],
    ...
    visualReview: c.visualReview, assets: c.assets.map(a => ({ id: a.id, purpose: a.purpose, authoring: a.authoring, source: a.source, editable: a.editable, sourceFormat: a.sourceFormat ?? 'unspecified-legacy', ai: a.provenance.ai, size: [a.width, a.height] })) };
...
function snapshot(root) {
  ...
  for (const name of new Set(['design/requirements.json', ...c.references, ...c.assets.flatMap(a => [a.source, a.editable, ...generationFiles(root, a)])])) inputs[name] = hash(read(root, name));
...
export function build(root) {
  return locked(root, () => {
    ...
    for (const a of s.c.assets) {
      ...
      if (a.provenance.ai) {
        for (const original of new Set([a.provenance.generation.original.path, ...(a.sourceFormat === 'flat-png' ? [a.editable] : [])])) {
          const bytes = read(root, original);
          check(bytes.length >= 33 && bytes.subarray(0, 8).equals(Buffer.from([137,80,78,71,13,10,26,10])), 'Original/working source is not PNG');
          const width = bytes.readUInt32BE(16), height = bytes.readUInt32BE(20);
          check(width > 0 && height > 0 && width <= 4096 && height <= 4096 && width * height <= 4194304, 'Original generated image exceeds bounds');
          inspect(s.tools.imagemagick.binary, root, original, { id: a.id, width, height, alpha: 'any', maxColors: 16777216 });
        }
      }
      write(stage, `pack/${a.id}.png`, read(root, a.source), true);
      assets.push({ id: a.id, file: `${a.id}.png`, ...metadata, ...(a.grid ? { grid: a.grid } : {}), provenance: a.provenance, source: a.source, editable: a.editable, sourceFormat: a.sourceFormat ?? 'unspecified-legacy' });
    }
    // Preserve all originals AND transitive generation/cost/processing evidence.
    for (const ref of Object.keys(s.data.inputs)) write(stage, `sources/${ref}`, read(root, ref), true);
