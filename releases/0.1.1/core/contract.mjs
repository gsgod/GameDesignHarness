...
export function validate(c) {
  ...
  check(c.policy?.network === false && c.policy?.paidTools === false && c.policy?.generativeAI === false, 'v0.1 is offline, free-tool, non-generative only');
  ...
  for (const a of c.assets) {
    ...
    check(p && ['original', 'licensed', 'pending'].includes(p.kind) && text(p.creator) && text(p.license) && text(p.source) && p.ai === false, 'Provenance fields required');
